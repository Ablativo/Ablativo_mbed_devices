#ifndef __BLE_BEACON_SERVICE_H
#define __BLE_BEACON_SERVICE_H
...
class Beacon : ble::Gap::EventHandler {
public:
    Beacon(BLE &ble, events::EventQueue &event_queue) :
        _ble(ble),
    ...
    void start() {
        ...
        _ble.init(this, &Beacon::on_init_complete);
...
private:
    ...
    void start_advertising() {
         ...
         * 128-Bit / 16byte UUID = E2 0A 39 F4 73 F5 4B C4 A1 2F 17 D1 AD 07 A9 61  ->  random for testing
        ...
        uint16_t major_number = MBED_CONF_APP_MUSEUM_ID;
        uint16_t minor_number = MBED_CONF_APP_DEVICE_ID;
        uint16_t tx_power     = 0xC8;
        uint16_t comp_id      = 0x004C; //Apple
